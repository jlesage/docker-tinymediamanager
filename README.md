...
services:
  tinymediamanager:
    image: jlesage/tinymediamanager
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/tinymediamanager:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-tinymediamanager/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/tinymediamanager/tags
...
```shell
docker pull jlesage/tinymediamanager
```
...
For Synology NAS users, follow these steps to update a container image:
  ...
  3.  In the search bar, type the name of the container (`jlesage/tinymediamanager`).
  4.  Select the image, click *Download*, and choose the `latest` tag.
  5.  Wait for the download to complete. A notification will appear once done.
  6.  Click *Container* in the left pane.
  7.  Select your tinyMediaManager container.
...
container, it would proxy all HTTP requests for `tinymediamanager.domain.tld` to
...
upstream docker-tinymediamanager {
...
server {
	...
	location / {
		proxy_pass http://docker-tinymediamanager;
...
| **Application version** label | Displays the version of tinyMediaManager integrated into Docker image. |
...
Any desktop notification generated by tinyMediaManager is forwarded to the
